...
{
    /// <summary>
    /// 凤凰反转 (Phoenix Reversal) —— 双引擎反转交易系统
    ///
    ...
    /// </summary>
    public class PhoenixReversal : StgBase
    {
        public PhoenixReversal()
        {
        }

        public PhoenixReversal(string id) : base(id)
        {
        }
        ...
        public override StgDesc GetStgDesc()
        {
            ...
            // 出场风格
            sd.ArgDic["exitStyle"] = 1;             // 0:胜率型(分批+吊灯) 1:波段型(拿到对侧轨道,唐奇安式)
            sd.ArgDic["sarMode"] = 0;               // SAR反手：波段目标出场时立即反向开仓（收益更高但近乎永远在场，默认关闭）
            ...
            sd.ArgDescDic["exitStyle"] = new ArgDesc() { Text = "出场风格", Explain = "波段型：持满仓位直至触及对侧布林轨道，单笔吃满整段回归（唐奇安反转式，收益最大）；胜率型：分批止盈+吊灯移动止损，胜率高但单笔小", Options = "0:胜率型|1:波段型", Type = "select" };
            sd.ArgDescDic["sarMode"] = new ArgDesc() { Text = "SAR反手", Explain = "波段目标出场的同时立即反向开仓，永远在场吃回程行情（受交易方向mode约束：单向模式下不反手）", Options = "0:关闭|1:开启", Type = "bool" };
        ...
        public override void OnBar(Period period, TableUnit tu, bool isFinal, SkQuote tq)
        {
            ...
            int exitStyle = Convert.ToInt32(ArgDic["exitStyle"]);
            int sarMode = Convert.ToInt32(ArgDic["sarMode"]);
            ...
            if (s.Status == 0)
            {
                ...
                // （多头止损后需快RSI回升过50——不是同一把飞刀重接，而是新一轮行情）
                if (s.NeedReset == 1 && (rsiFast != null ? rsiFast.Value >= 50 : q.Close >= middle)) s.NeedReset = 0;
                if (s.NeedReset == 2 && (rsiFast != null ? rsiFast.Value <= 50 : q.Close <= middle)) s.NeedReset = 0;

                // ===== 通道破位触发（事件驱动，与唐奇安同源）=====
                // 创N根新低/新高本身就是"极端"的自适应定义，不受带宽/制度过滤限制以保证参与度，
                // 但同样遵守止损冷却与周期复位守卫（不反复接同一把飞刀）
                bool breachLong = false, breachShort = false;
                if (useBreach == 1 && !inCooldown)
                {
                    ...
                    breachLong = mode != 2 && s.NeedReset != 1 && q.Close < chLow;
                    breachShort = mode != 1 && s.NeedReset != 2 && q.Close > chHigh;
                    if (breachLong && breachShort) { breachLong = false; breachShort = false; }
                }

                bool gatesOk = !inCooldown && bandWidthOk;

                // ===== 狙击引擎信号 =====
                // 位置门槛：至少偏离均值ZGate个标准差；确认K线：反转由收盘价确认，不接飞刀；
                // 制度门控：趋势态（方差比过高）不开反转仓
                bool longSignal = gatesOk && regimeOk && mode != 2 && s.NeedReset != 1
                    && z <= -ZGate && bullScore >= minScore && q.Close > prevQ.Close;
                bool shortSignal = gatesOk && regimeOk && mode != 1 && s.NeedReset != 2
                    && z >= ZGate && bearScore >= minScore && q.Close < prevQ.Close;

                // 双向模式下同时触发（极罕见），取分数更高的一边
                if (longSignal && shortSignal)
                {
                    if (bullScore >= bearScore) shortSignal = false;
                    else longSignal = false;
                }

                // ===== 脉冲引擎信号（狙击未触发时）=====
                bool pulseLong = false, pulseShort = false;
                if (gatesOk && regimeOk && !longSignal && !shortSignal && pulseMode != 0 && rsiFast != null)
                {
                    ...
                    pulseLong = mode != 2 && s.NeedReset != 1 && trendLongOk
                        && (pulseMode == 1 ? rsiLongOk && ibsLongOk : rsiLongOk || ibsLongOk);
                    pulseShort = mode != 1 && s.NeedReset != 2 && trendShortOk
                        && (pulseMode == 1 ? rsiShortOk && ibsShortOk : rsiShortOk || ibsShortOk);
                    if (pulseLong && pulseShort) { pulseLong = false; pulseShort = false; }
                }
                ...
                else if (breachLong || breachShort)
                {
            ...
            {
                if (exitStyle == 1)
                    ProcessSwingExit(s, tu, q, true, rsiFast, upper, lower,
                        atrValue, atrMult, maxStopPct, rsiValue, pulseAdds, sniperAdds,
                        sarMode, mode, sendMode, period);
                else if (s.EntryPath == 2)
                    ProcessPulseExit(s, tu, q, true, rsiFast, pulseExitSma, effPulseMaxHold,
            ...
            {
                if (exitStyle == 1)
                    ProcessSwingExit(s, tu, q, false, rsiFast, upper, lower,
                        atrValue, atrMult, maxStopPct, rsiValue, pulseAdds, sniperAdds,
                        sarMode, mode, sendMode, period);
                else if (s.EntryPath == 2)
                    ProcessPulseExit(s, tu, q, false, rsiFast, pulseExitSma, effPulseMaxHold,
                        pulseAdds, pulseRunner, pulseRsiBuy, upper, lower, atrValue, atrMult, maxStopPct,
                        trailAtrMult, minProfitAtr, sendMode, period);
                else
        ...
        /// <summary>
        /// 全部平仓并记录诊断：出场原因、分引擎胜负、持仓周期；亏损离场且指定冷却时进入冷却期
        /// </summary>
            ...
            Period period, int sendMode, int cooldownBars)
        {
            if (s.Num > 0)
            {
                ...
                s.Realized += isLong ? (price - s.AvgPrice) * s.Num : (s.AvgPrice - price) * s.Num;
            }
            Diag("exit_" + reason);
            ...
            Diag("hold_n");
            bool wasLoss = s.Realized <= 0;
            bool needReset = wasLoss && reason == "stop";
            ResetState(s);
            if (wasLoss && cooldownBars > 0) s.CooldownRemain = cooldownBars;
            // 止损离场后启动周期复位守卫，防止立即重接同一把飞刀
            if (needReset) s.NeedReset = isLong ? 1 : 2;
        }
