		...
		public override void OnBar(Period period, TableUnit tu, bool isFinal,SkQuote tq)
        {
            ...
            if (isFinal)
			{
				if (tu.QuoteList.Count > 1)
				{
                    ...
                    var rsi=tu.QuoteList.GetRsi((int)ArgDic["lookbackPeriods"]).ToList();
					...
					if (s.Status == 0)
					{
						if ((decimal)rsi1.Rsi< overDown && mode!=2)
						{
                            s.Status = 1;
                            s.Num = num;
                            Trade(tu.MktSymbol, OrderType.BUY, q.Close, num, period,sendMode);
                        }
						if ((decimal)rsi1.Rsi > overUp && mode != 1)
						{
                            s.Status = 2;
                            s.Num = num;
