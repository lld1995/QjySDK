...
namespace QjySDK.Stg
{
    public class KDJ : StgBase
    {
        ...
        public override StgDesc GetStgDesc()
        {
            ...
            sd.ArgDic["overUp"] = 80m;
            sd.ArgDic["overDown"] = 20m;
        ...
        public override void OnBar(Period period, TableUnit tu, bool isFinal, SkQuote tq)
        {
            ...
            if (isFinal)
            {
                if (tu.QuoteList.Count > 15)
                {
                    ...
                    if (s.Status == 0)
                    {
                        if ((decimal)kdj1.K < overDown && (decimal)kdj1.D < overDown && kdj2.K < kdj2.D && kdj1.K > kdj1.D && mode != 2)
                        {
                            s.Status = 1;
                            s.Num = num;
                            Trade(tu.MktSymbol, OrderType.BUY, q.Close, num, period, sendMode);
                        }
                        if ((decimal)kdj1.K > overUp && (decimal)kdj1.D > overUp && kdj2.K > kdj2.D && kdj1.K < kdj1.D && mode != 1)
                        {
                            s.Status = 2;
                            s.Num = num;
