...
namespace QjySDK.Stg
{
    ...
    public class MACD_ADX_Resonance : StgBase
    {
            ...
            decimal atr, SignalAnalysis signals, double adxValue)
        {
            ...
            if (_useTrailingStop && state.TrailingStopActivated)
            {
                if (currentLow <= state.TrailingStopPrice)
                {
                    ClosePosition(state, mktSymbol, state.TrailingStopPrice, period, "TrailingStop");
                    return;
                }
            }
            ...
            decimal atr, SignalAnalysis signals, double adxValue)
        {
            ...
            if (_useTrailingStop && state.TrailingStopActivated)
            {
                if (currentHigh >= state.TrailingStopPrice)
                {
                    ClosePosition(state, mktSymbol, state.TrailingStopPrice, period, "TrailingStop");
                    return;
                }
        ...
        {
            Trade(mktSymbol, OrderType.BUY, price, _tradeAmount, period, _sendMode);
        ...
        private void OpenShortPosition(TradeState state, string mktSymbol, decimal price, decimal atr,
            Period period, double macdValue, double adxValue)
        {
            Trade(mktSymbol, OrderType.SELL, price, _tradeAmount, period, _sendMode);
