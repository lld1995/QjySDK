...
namespace QjySDK.Stg
{
	...
	public class ChanLunBi : StgBase
	{
		...
		public override StgDesc GetStgDesc()
		{
			...
			// 止损设置（笔级别信号密集，依赖买卖点信号自然反转，不使用百分比止损）
			...
			// 信号过期设置
			sd.ArgDic["signalExpiryBars"] = 0;  // 笔级别不使用信号过期（笔间距短，过期反而过滤有用信号）

			// 中枢回归平仓（笔级别信号密集，中枢退出易过早平仓，默认关闭）
			sd.ArgDic["useZhongShuExit"] = 0;    // 价格回到中枢内部时平仓（0否 1是）
		...
		/// <summary>
		/// 判断是否存在上涨趋势
		/// 优先严格定义（中枢无重叠），其次放宽为中枢中心上移且高低点递升
		/// </summary>
