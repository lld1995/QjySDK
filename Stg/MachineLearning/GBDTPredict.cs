...
{
    /// <summary>
    /// 基于GBDT(梯度提升决策树)的价格预测交易策略
    /// 策略逻辑：
    /// - 使用GBDT预测下一根K线的价格走势
    /// - 预测上涨则做多，预测下跌则做空
    /// - 结合ATR进行止损止盈管理
    /// GBDT特点：
    /// - 使用一阶梯度进行优化（与XGBoost使用二阶泰勒展开不同）
    /// - 每棵树拟合前一轮的残差
    /// - 通过学习率控制每棵树的贡献
    /// - 支持多种损失函数
    /// </summary>
    public class GBDTPredict : StgBase
    {
        ...
        public override StgDesc GetStgDesc()
        {
            ...
            sd.ArgDic["learningRate"] = 0.1;         // 学习率（收缩系数）
            sd.ArgDic["minSamplesLeaf"] = 5;         // 叶子节点最小样本数
            sd.ArgDic["minSamplesSplit"] = 10;       // 分裂所需最小样本数
            sd.ArgDic["subsample"] = 0.8;            // 样本采样比例
            sd.ArgDic["maxFeatures"] = 0.8;          // 特征采样比例
            ...
            sd.ArgDescDic["numTrees"] = new ArgDesc() { Text = "树数量", Explain = "GBDT中树的数量" };
            sd.ArgDescDic["maxDepth"] = new ArgDesc() { Text = "最大深度", Explain = "决策树的最大深度" };
            sd.ArgDescDic["learningRate"] = new ArgDesc() { Text = "学习率", Explain = "模型训练学习率(收缩系数)" };
            sd.ArgDescDic["minSamplesLeaf"] = new ArgDesc() { Text = "叶子最小样本", Explain = "叶子节点最小样本数" };
            sd.ArgDescDic["minSamplesSplit"] = new ArgDesc() { Text = "分裂最小样本", Explain = "分裂所需最小样本数" };
            sd.ArgDescDic["subsample"] = new ArgDesc() { Text = "样本采样", Explain = "每棵树的样本采样比例" };
            sd.ArgDescDic["maxFeatures"] = new ArgDesc() { Text = "特征采样", Explain = "每棵树的特征采样比例" };
        ...
        /// GBDT决策树节点
        /// </summary>
        private class GBDTTreeNode
        {
            public int FeatureIndex { get; set; } = -1;
            public double SplitValue { get; set; }
            public double LeafValue { get; set; }
            public bool IsLeaf { get; set; } = true;
            public GBDTTreeNode? Left { get; set; }
            public GBDTTreeNode? Right { get; set; }
        }
        ...
        /// 使用均方误差作为分裂准则
        /// </summary>
        private class GBDTRegressionTree
        {
            ...
            private GBDTTreeNode? _root;
            private int[]? _selectedFeatures;
            private int _numFeatures;

            public GBDTRegressionTree(int maxDepth, int minSamplesLeaf, int minSamplesSplit, double maxFeatures, Random random)
            {
                _maxDepth = maxDepth;
            ...
            public void Fit(double[][] X, double[] residuals, int[] sampleIndices)
            {
                ...
                // 特征采样：随机选择特征子集
                int numSelectedFeatures = Math.Max(1, (int)(_numFeatures * _maxFeatures));
            ...
            private GBDTTreeNode BuildTree(double[][] X, double[] residuals, int[] indices, int depth)
            {
                var node = new GBDTTreeNode();
                ...
                // 检查是否应该成为叶子节点
                if (depth >= _maxDepth || indices.Length < _minSamplesSplit || indices.Length < 2 * _minSamplesLeaf)
                {
                    node.IsLeaf = true;
                    node.LeafValue = mean;
                    return node;
            ...
            private double PredictNode(GBDTTreeNode node, double[] x)
            {
            ...
            private void CollectFeatures(GBDTTreeNode? node, HashSet<int> features)
            {
        ...
        /// 2. 每一轮计算负梯度（残差）
        /// 3. 用回归树拟合残差
        /// 4. 更新模型预测 = 上一轮预测 + 学习率 * 新树预测
        /// </summary>
        private class GBDTModel
        {
            private List<GBDTRegressionTree> _trees;
            ...
            public GBDTModel(int numTrees, int maxDepth, double learningRate, int minSamplesLeaf,
                int minSamplesSplit, double subsample, double maxFeatures)
            {
                _numTrees = numTrees;
                _maxDepth = maxDepth;
                ...
                _maxFeatures = maxFeatures;
                _trees = new List<GBDTRegressionTree>();
            ...
            public void Train(double[][] X, double[] y)
            {
                ...
                for (int t = 0; t < _numTrees; t++)
                {
                    ...
                    // 训练新树拟合残差
                    var tree = new GBDTRegressionTree(_maxDepth, _minSamplesLeaf, _minSamplesSplit, _maxFeatures, _random);
        ...
        /// GBDT特征提取器
        /// </summary>
        private static class GBDTFeatureExtractor
        {
            /// <summary>
            /// 提取技术指标特征
            /// </summary>
        ...
        private class State
        {
            ...
            public GBDTModel? Model { get; set; }   // GBDT模型
        ...
        public override void OnBar(Period period, TableUnit tu, bool isFinal, SkQuote tq)
        {
            ...
            int minSamplesLeaf = (int)ArgDic["minSamplesLeaf"];
            int minSamplesSplit = (int)ArgDic["minSamplesSplit"];
            double subsample = Convert.ToDouble(ArgDic["subsample"]);
            double maxFeatures = Convert.ToDouble(ArgDic["maxFeatures"]);
            ...
            if (s.Model == null)
            {
                s.Model = new GBDTModel(numTrees, maxDepth, learningRate, minSamplesLeaf, minSamplesSplit, subsample, maxFeatures);
            }
            ...
            {
                var features = GBDTFeatureExtractor.ExtractFeatures(tu.QuoteList, lookback);
        ...
        private (double[][] X, double[] y) PrepareTrainingData(List<SkQuote> quotes, int lookback, int trainPeriod)
        {
            ...
            for (int i = startIdx; i < endIdx; i++)
            {
                ...
                var features = GBDTFeatureExtractor.ExtractFeatures(subQuotes, lookback);
